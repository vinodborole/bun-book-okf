...
`TextEncoder#encode`
[, two utility classes that translate between strings and various binary encodings, most notably](https://developer.mozilla.org/en-US/docs/Web/API/TextDecoder)
...
Streams let you work with binary data without loading it all into memory at once. They’re commonly used for reading and writing files, sending and receiving network requests, and processing large amounts of data. Bun implements the Web APIs
...
`ArrayBuffer` in chunks, use a `Uint8Array` view and enqueue each chunk.
### From `TypedArray`
...
`ArrayBuffer` in chunks, split the `TypedArray` into chunks and enqueue each one individually.
### From `DataView`
...
`ArrayBuffer` in chunks, split the `DataView` into chunks and enqueue each one individually.
### From `Buffer`
...
`ArrayBuffer` in chunks, split the `Buffer` into chunks and enqueue each one individually.
### From `Blob`
