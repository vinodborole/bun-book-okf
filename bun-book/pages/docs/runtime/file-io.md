...
`Bun.file` and `Bun.write` APIs documented on this page are heavily optimized and are the recommended way to work with files in Bun. For operations they don’t cover, such as `mkdir` or `readdir`, use Bun’s [nearly complete](/docs/runtime/nodejs-compat#node-fs)implementation of the[module.](https://nodejs.org/api/fs.html)`node:fs`
...
`Blob`
[file descriptors](https://en.wikipedia.org/wiki/File_descriptor)or
